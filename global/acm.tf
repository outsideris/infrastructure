...
resource "aws_route53_record" "wild_sideeffect_kr_validation" {
  name    = tolist(aws_acm_certificate.wild_sideeffect_kr.domain_validation_options).0.resource_record_name
  type    = tolist(aws_acm_certificate.wild_sideeffect_kr.domain_validation_options).0.resource_record_type
  zone_id = aws_route53_zone.sideeffect_kr.zone_id
  records = [tolist(aws_acm_certificate.wild_sideeffect_kr.domain_validation_options).0.resource_record_value]
  ttl     = 60
}
